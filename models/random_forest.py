...
class RandomForest:
    ...
    def _fit(self) -> tuple[DataFrame, DataFrame, Index, Index]:
        """ Fit and test a Random Forest classifier via cross validation """
        # Cross-validation
    ...
    def _predict(self, x_test: DataFrame, y_test: Index):
        ...
        logging.info(f"Accuracy of Random Forest: {accuracy}")
        logging.info(f"Confusion matrix:\n {conf_matrix.to_markdown(index=False)}")

    def random_forest(self):
        """ Fit the Random Forest model and predict against the test set via cross-validation """
